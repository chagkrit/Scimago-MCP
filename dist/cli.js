...
program
    ...
    .option("--categories <categories>", `Comma-separated categories: ${scimagoDefaultCategories.join(", ")}`, parseCategories, [...scimagoDefaultCategories])
...
program
    ...
    const rows = toResearchCsvRows(await client.search({
        query: options.query,
        maxResults: options.maxResults,
        count: options.count,
        view: options.view,
...
program
    ...
    .action(async (options) => {
    const client = new ScopusClient(getConfig());
    const rows = toSourceCsvRows(await client.sources({
        title: options.title,
        issn: options.issn,
...
function parseCategories(value) {
    return parseCsvValues(value, [...scimagoDefaultCategories], "category");
}
function parseQuartiles(value) {
    return parseCsvValues(value.toUpperCase(), [...scimagoQuartiles], "quartile");
}
function parseCsvValues(value, allowed, label) {
    const values = value
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean);
