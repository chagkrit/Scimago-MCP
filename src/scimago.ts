...
export const scimagoQuartiles = ["Q1", "Q2", "Q3", "Q4"] as const;
...
export class ScimagoClient {
  ...
  async searchJournals(options: ScimagoSearchOptions = {}): Promise<ScimagoJournalRow[]> {
    ...
    const rows: ScimagoJournalRow[] = [];

    for (const category of categories) {
      const categoryConfig = scimagoCategoryMap[category];
      const url = this.buildJournalRankUrl(category, options);
      const text = await this.fetchText(url);
      const parsedRows = parseScimagoCsv(text, categoryConfig, url.toString()).filter((row) =>
        quartiles.has(row.quartile as ScimagoQuartile),
      );
  ...
  buildJournalRankUrl(category: ScimagoCategory, options: ScimagoSearchOptions = {}): URL {
    ...
    if (sourceType !== "all") url.searchParams.set("type", sourceType);
    if (options.openAccess !== undefined) url.searchParams.set("openaccess", String(options.openAccess));
    if (options.wos !== undefined) url.searchParams.set("wos", String(options.wos));
    if (options.order) url.searchParams.set("order", options.order);
    if (options.direction) url.searchParams.set("ord", options.direction);
...
export const scimagoColumns: Array<keyof ScimagoJournalRow> = [
  "rank",
  "source_id",
  "title",
  "type",
...
function parseScimagoCsv(
  text: string,
  category: (typeof scimagoCategoryMap)[ScimagoCategory],
  url: string,
): ScimagoJournalRow[] {
